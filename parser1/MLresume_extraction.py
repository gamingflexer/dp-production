...
import pandas as pd
import re
from parser1.commons import constants as com
import os
import string
import nltk
...
def get_custom_skills(text):
    skill_set = list()
    try:
        with open(config_p.skill_rb, 'rb') as fp:
...
def get_skills(custom_entities, text):
    ...
    try:
        model_dir = config_p.skill_model_dir
    ...
    for ind, line in enumerate(lines):
        if len(line.split()) < 4 and ('skills' in line.lower()):
            try:
                ...
                exp.append(lines[ind+i])
    description = [e for e in exp if not e[:5].lower() ==
                   'level' and len(e) > 0]
    exp = get_head_sections(text)['skills']
    if len(exp.split('\n')) < 4:
        exp = ''
    ...
    data = pd.read_csv(config_p.skill_csv)
...
def get_head_sections(data, e_val=30, split_val=10):
    ...
    for index, line in enumerate(data.split('\n')):
        ...
        if len(line.split()) < split_val:
            ...
            splitted = [l[:-1] for l in line.lower().split()
                        if len(ln.lower().split()) >= 1]
            splitted = splitted[:3]
    ...
    sections_dict = {"experience": '', "skills": '', "education": '',
                     'trainings': '', 'publications': '', 'awards': '', 'project': ''}
...
def get_location(text, ents):
    nations = pd.read_csv(config_p.nations)
    country = pd.read_csv(config_p.country)
    state = pd.read_csv(config_p.state)
    ...
    loc_extended = []
    with open(config_p.city, 'r') as f:
        for line in f:
            cities = line.split(',')
    ...
    locations_captured = list(set(
        [(entity) for sent in text.sents for entity in sent.ents if entity.label_ == 'GPE']))
...
def get_phonenumber(text):
    ...
    validated_number = []
    data = pd.read_csv('parser1/commons/phoneno.csv')
    for num in number:
