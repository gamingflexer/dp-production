...
custom_nlp3 = spacy.load(os.path.join(base_path,"company","model")) ###########################################
...
designitionmatcher = PhraseMatcher(en_sm.vocab)
...
file = os.path.join(base_path,"skills2.txt")
file = open(file, "r", encoding='utf-8')    
skill = [line.strip().lower() for line in file]
skillsmatcher = PhraseMatcher(en_sm.vocab)
patterns = [en_sm.make_doc(text) for text in skill if len(en_sm.make_doc(text)) < 10]
...
class resumeparse(object):
    ...
    def extract_skills(text):
        ...
        __nlp = en_sm(text.lower())
        # Only run en_sm.make_doc to speed things up
